...
The answer is, unsurprisingly, [Transplant](https://github.com/bastibe/transplant), my bridge between Matlab and Python. To be fair, it's a one-way bridge, which allows Python to call Matlab, but not vice versa.
...
And frankly, this stuff should be easy. There are bridges between many languages, and typically they are not particularly complicated. But we are talking about Matlab here, and while Matlab has a C API, it is not powerful enough for our purposes. So how about we just use sockets instead? Well, sockets are only included with the [Instrument Control Toolbox](https://www.mathworks.com/help/instrument/communicate-using-tcpip-server-sockets.html). And let's just not talk about how some Matlab errors can't be caught with `try/catch`.

But these are technical problems, and technical problems can be solved. Particularly, Transplant uses [ZeroMQ](http://zeromq.org/) sockets for communications[fn:ZMQ], and just re-starts its main loop in `atexit` if an uncatchable error slipped by.
