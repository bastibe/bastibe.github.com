...
signal[int(samplerate * 0.21)] = 1
signal[int(samplerate * 0.22)] = 1

time = numpy.arange(samplerate)/samplerate
ax1.plot(time, signal)
ax1.set(xlim=[0.19, 0.23])
ax1.set(ylabel='Amplitude', xlabel='Time in s', title='Waveform')

for idx, f in enumerate(numpy.arange(100, 550, 50)):
    ax2.plot(time, 0.02*numpy.cos(time*numpy.pi*2*f + numpy.pi*2*f*0.2)+idx/20, color='C0', alpha=0.5)
for idx, f in enumerate(numpy.arange(100, 550, 50)):
    ax2.plot(time, 0.02*numpy.cos(time*numpy.pi*2*f + numpy.pi*2*f*0.21)+idx/20, color='C1', alpha=0.5)
ax2.plot([0.2, 0.2], [-0.02, 0.42], linestyle='--', color='black', linewidth=0.5)
ax2.plot([0.21, 0.21], [-0.02, 0.42], linestyle='--', color='black', linewidth=0.5)
ax2.plot([0.22, 0.22], [-0.02, 0.42], linestyle='--', color='black', linewidth=0.5)

ax2.set(xlim=[0.19, 0.23])
