...
class WorldMap(QtWidgets.QGraphicsView):
    ...
    def mouseMoveEvent(self, event):
        # Qt 6 deprecates event.pos() which returns an integer QPoint
        # in favor of event.position(), which returns a floating point
        # QPointF, since modern high-resolution computer displays
        # require coordinate calculations in fractional pixels.
        # However, the QGraphicsScene.itemAt() function still requires
        # a QPoint, not a QPointF, thus we add a conversion here:
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        world_map.currentCountry.connect(self.set_status_bar_message)
