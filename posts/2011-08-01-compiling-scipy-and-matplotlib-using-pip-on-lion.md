...
But, that is not enough yet. As [this blogpost](http://jholewinski.wordpress.com/2011/07/21/installing-matplotlib-on-os-x-10-7-with-homebrew/) outlines, Matplotlib is not currently compatible with `libpng` 1.5, which ships with Lion. Fortunately, this is already fixed in the most recent source on the Matplotlib repo, so you just have to checkout that:
