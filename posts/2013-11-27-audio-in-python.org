...
Of course, there is much more you could do. You could create a callback function and be called every four[fn:1] frames with new audio data to process. You could request your audio data as ~int16~, because that would be totally awesome! You could use many different sound cards at the same time, and route stuff to and fro to your hearts desire! And you can run all this on Linux using ALSA or Jack, or on Windows using DirectSound or ASIO, or on Mac using CoreAudio[fn:2]. And you already saw that you can read Wave files, OGG, FLAC or MAT-files[fn:3].
...
[fn:1] You can use any block size you want. Less than 4 frames per block can be really taxing for your CPU though, so be careful or you start dropping frames.
[fn:2] More precisely: Everything that [[http://www.portaudio.com/][portaudio]] supports.
[fn:3] More precisely: Everything that [[http://www.mega-nerd.com/libsndfile/][libsndfile]] supports.
