...
with relative ease. The process goes like this: Download [Ukelele (free)](https://software.sil.org/ukelele/) to create a new keyboard
layout bundle for your base layout[^ukelele]. Inside that bundle, there's a _*.keylayout_ file,
...
Windows and Linux before, and for the life of me _could not do it_. So I actually think this is
great!

[^ukelele]: if there's an easier way to create a keyboard layout bundle, please let me know. I
...
available on Linux as well using xinput[^xinput], but I haven't gotten the setting to stick across
reboots or sleep cycles. So I wrote a background task that checks xinput every five seconds,
which does the trick.

[^trackpads]: The Logitech T650 is actually not bad, but the drivers are a travesty. Some Wacom
tablets include touch, too, but they palm rejection is abysmal and they don't support gestures.

[^xinput]: `xinput set-prop "Kensington Slimblade Trackball" "libinput Scroll Method Enabled" 0, 0, 1 # button scrolling`   
...
CMD-Tab to switch between _apps_, now windows. And then there's another shortcut for switching
between windows of the same app, but the shortcut really doesn't work at all on a German keyboard.
Who came up with this nonsense? Anyway, [Witch ($14)](https://manytricks.com/witch/) implements window switching properly.
...
[Groupy ($12)](https://www.stardock.com/products/groupy/) (set it to only apply to explorer.exe). Windows 11 has very recently added native tabs,
...
Oh, and I always remove the iCloud/OneDrive sidebar entries, which is [surprisingly tedious](https://www.howtogeek.com/225973/how-to-disable-onedrive-and-remove-it-from-file-explorer-on-windows-10/) on
Windows.

## Hardware Control

On laptops, you can control screen brightness from your keyboard. On desktops, you can not. However,
some clever hackers have put together [BetterDisplay (OSS for screen brightness)](https://github.com/waydabber/BetterDisplay#readme), which adds this
...
Less great is that MacOS does not allow volume control on external sound cards. [SoundSource ($47)](https://rogueamoeba.com/soundsource/)
...
MacOS only supports _reading_ NTFS, but no writing. [Paragon NTFS (€20)](https://www.paragon-software.com/home/ntfs-mac/#) adds this with another kernel
...
Absolutely astonishingly, the Steam Deck runs Windows games on _Linux_ through emulation, which
...
[^VMs]: VirtualBox does not have a good accelerated driver, and VMWare does not support recent
kernels. Qemu _should_ be able to solve this problem, but I couldn't get it to work reliably.
...
especially for its lack of access to commercial applications. While Linux _should_ be the most
