...
First up, *resolution*: I printed a [[https://www.graphics.cornell.edu/~westin/misc/res-chart.html][test chart]] on A3 paper. First I tried printing the PDF directly with Preview.app, but noticed that the chart itself started blurring lines at 1250 L/PH[fn::lines per picture height], which implies that Preview.app prints at 1250/8.7in≈150DPI[fn::the height of the measuring area was 8.7in (of the 11.7in paper)]. Not satisfied, I exported the test chart to a high-DPI tiff, and printed with Canon's print application, which appropriately resolved the finest lines on the test chart.
...
Next to the resolution is the *contrast level* in ΔL[fn::I calibrated the white level to L=95, and ΔL is the difference between the white level and the black level on these lines.] (HSL), measured at the right side of this pack of lines, which is a good proxy for contrast of high frequency detail, or microcontrast. A lower black level means more microcontrast. Indeed, this is where the new lens shows a clear improvement over the other two lenses, with visibly more contrast in fine details.
