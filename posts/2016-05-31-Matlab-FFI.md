...
So, you grab your towel, you bite the bullet, you strap into your K&R, and get down to it: You start writing a Mex file. And you curse, and you cry, because writing C is hard, and Mex doesn't exactly make it any better. But you know what? _There is a better way!_

Because, unbeknownst to many, Matlab includes a Foreign Function Interface. The technique was probably pioneered by [Common Lisp](https://common-lisp.net/project/cffi/), and has since [been](http://luajit.org/ext_ffi.html) [widely](http://cffi.readthedocs.io/en/latest/overview.html) [adopted](http://docs.oracle.com/javase/7/docs/technotes/guides/jni/spec/functions.html) [everywhere](https://en.wikipedia.org/wiki/Foreign_function_interface): calling functions in a C library without writing any C code and without invoking a compiler!
...
However, If I ask myself whether I would sacrifice 0.00007 seconds of computational overhead for hours of my life _not_ spent with Mex, there really is no competition. I will choose Matlab's FFI over writing Mex files every time.
