...
Our first stop in speech analysis is usually the short-time Fourier transform, or STFT[^1]:

<figure>
<img src="https://bastibe.de/static/2019-09/stft.png" />
<caption>Top: an STFT of speech (it's me, saying: "Es war einmal ein Mann"). Bottom: the signal's waveform.</caption>
</figure>
...
All of this is clearly visible in this STFT. To calculate the STFT, we chop up the audio signal into short, overlapping blocks, apply a _window function_ [^2] to each of these blocks, and Fourier-transform each windowed block into the frequency domain:

<figure>
<img src="https://bastibe.de/static/2019-09/block-processing.png" />
<caption>The STFT is a two-dimensional graph of short-time spectra. Window functions and corresponding spectra of some blocks are highlighted.</caption>
</figure>

We zoomed in a bit to make this visible. At this zoom level, an [old friend](https://bastibe.de/2019-09-19-periodicity-and-harmonicity.html) becomes visible in the waveform: The waveform of our harmonic spectra is clearly periodic. We also see that each of our blocks contain multiple periods of the speech signal. So what happens if we make our blocks shorter?

<figure>
<img src="https://bastibe.de/static/2019-09/short-blocks.png" />
<caption>STFT with shorter blocks (graph shows a shorter time, too). Each window now covers roughly one period of the signal.</caption>
</figure>
...
<figure>
<img src="https://bastibe.de/static/2019-09/cqt.png" />
<caption>The CQT uses shorter block sizes for higher frequencies and longer block sizes for lower frequencies.</caption>
</figure>
...
<figure>
<img src="https://bastibe.de/static/2019-09/gammatone.png" />
<caption>A gamma-tone filterbank is a series of band-pass filters. Each horizontal line is now a frequency-constrained waveform.</caption>
</figure>

The fundamental frequency is now clearly a visible as a single sinusoid, rotating its merry way at 120 Hz. Each harmonic is its own sinusoid at an integer multiple of the fundamental frequency, but changing in amplitude in lockstep with the findamental. Thus in this graph, we see both the harmonicity and the periodicity at the same time, and can finally appreciate the complex interplay of magnitudes and phases that give rise to the beautiful and simple sound of the human voice[^3].

[^1]: also known as "spectrogram"
[^2]: a Hann window, in this case
[^3]: in the magnitude domain only; Phases, and different window functions, go beyond the scope of this article.
