...
filetags: audio programming windows
---
...
It has long been a major frustration for my work that Python does not have a great package for playing and recording audio. My first step to improve this situation was a small contribution to [PyAudio](https://people.csail.mit.edu/hubert/pyaudio/), a CPython extension that exposes the C library [PortAudio](http:_www.portaudio.com_) to Python. However, I soon realized that PyAudio mirrors PortAudio's C API a bit too closely for comfort. Thus, I set out to write [PySoundCard](https://github.com/bastibe/PySoundCard), which is a higher-level wrapper for PortAudio that tries to be more pythonic and uses NumPy arrays instead of untyped `bytes` buffers for audio data. However, I then realized that PortAudio itself had some inherent problems that a wrapper would not be able to solve, and a truly great solution would need to do it the hard way:

Instead of relying on PortAudio, I would have to use the native audio APIs of the three major platforms directly, and implement a simple, cross-platform, high-level, NumPy-aware Python API myself. This effort resulted in [PythonAudio](https://github.com/bastibe/Python-Audio), a new pure-Python package that uses [CFFI](http://cffi.readthedocs.io/en/latest/) to talk to [PulseAudio](https://www.freedesktop.org/wiki/Software/PulseAudio/) on Linux, [Core Audio](https://developer.apple.com/library/content/documentation/MusicAudio/Conceptual/CoreAudioOverview/Introduction/Introduction.html) on macOS, and [WASAPI](https://msdn.microsoft.com/en-us/library/windows/desktop/dd371455(v=vs.85).aspx)[^1] on Windows.
...
[^1]: WASAPI is part of the Windows [Core Audio](https://msdn.microsoft.com/en-us/library/windows/desktop/dd370784(v=vs.85).aspx) APIs. To avoid confusion with the macOS API of the same name, I will always to refer to it as WASAPI.
...
WASAPI is one of several native audio libraries in Windows. PortAudio actually [supports five of them](http://portaudio.com/docs/v19-doxydocs/compile_windows.html): [Windows Multimedia (MME)](https://msdn.microsoft.com/en-us/library/windows/desktop/dd743883(v=vs.85).aspx), the first built-in audio API for Windows 3.1x; [DirectSound](https://msdn.microsoft.com/en-us/library/windows/desktop/ee416960(v=vs.85).aspx), the audio subsystem of DirectX for Windows 95;  [Windows Driver Model / Kernel Streaming (WDM/KS)](https://docs.microsoft.com/en-us/windows-hardware/drivers/stream/kernel-streaming), the improved audio system for Windows 98; [ASIO](https://en.wikipedia.org/wiki/Audio_Stream_Input/Output), a third-party API developed by Steinberg to make pro audio possible on Windows; and finally, [Windows Audio Session API (WASAPI)](https://msdn.microsoft.com/en-us/library/windows/desktop/dd370784(v=vs.85).aspx), introduced in Windows Vista to bring a modern audio API to Windows.
...
The documentation deserves even more praise: I have rarely seen such a well-documented API. There are high-level overview articles, there is commented example code, every object is described abstractly, and every method is described in detail and in reference to related methods and example code. There is no corner case that is left undescribed, and no error code without a detailed explanation. Truly, this is _exceptional_ documentation that is a joy to work with!

In conclusion, WASAPI leaves me in a situation I am very unfamiliar with: praising Windows. There is a non-trivial impedance mismatch between C and C# that has to be overcome to _use_ WASAPI from C. But once I understood this, the API itself and its documentation were easy to use and understand. Impressive!
