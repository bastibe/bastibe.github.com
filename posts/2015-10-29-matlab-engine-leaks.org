...
It seems that the ~array.array~ object is part of a ~matlab.double~ instance which is not referenced from anywhere but ~all_objects~. A classical memory leak.

After a bit of experimentation, I found the culprit. To illustrate, here's an example: The function ~leak~ passes some data to Matlab, and returns a float. Since the variables are not used outside of ~leak~, they should get deallocated when ~leak~ returns.
