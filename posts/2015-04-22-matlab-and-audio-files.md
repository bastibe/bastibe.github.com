...
So I wanted to work with audio files in Matlab. In the past, Matlab could only do this with `auread` and `wavread`, which can read _*.au_ and _*.wav_ files. With 2012b, Matlab introduced [`audioread`](https://www.mathworks.com/help/matlab/ref/audioread.html), which claims to support _*.wav_, _*.ogg_, _*.flac_, _*.au_, _*.mp3_, and _*.mp4_, and simultaneously deprecated `auread` and `wavread`.
