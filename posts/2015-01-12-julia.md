...
> Julia is a high-level dynamic programming language designed to address the requirements of high-performance numerical and scientific computing while also being effective for general purpose programming. —[Wikipedia](https://en.wikipedia.org/wiki/Julia_%28programming_language%29)
...
However, Julia is still very young, and still evolving rapidly. But if any of the above is true, I am _very_ interested!
...
It is actually nice to have a language that is built for scientific computation. Coming from Python, it is refreshing to have [array literals](http://docs.julialang.org/en/release-0.3/manual/arrays/), ranges and [mathematics](http://docs.julialang.org/en/release-0.3/stdlib/math/) available without importing anything. Much like Python, Julia also has a proper [module system](http://docs.julialang.org/en/release-0.3/manual/modules/), comprehensions, and more than one function per file.
...
And finally, Performance is surprisingly good! Although my algorithm spends almost all of its time doing FFTs, Julia performed about twice as fast as Python. This is pretty much exactly twice as much as I had expected! It also exposed more of the [features](http://docs.julialang.org/en/release-0.3/stdlib/math/?highlight=plan_fft#Base.plan_fft) of the underlying libraries, which could be used for another speed-up of some 10%.
