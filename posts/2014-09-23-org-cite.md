...
I am writing my master's thesis in [Org Mode](http:_orgmode.org_), and export to LaTeX for publishing. For the most part, this works incredibly well. Using Org Mode instead of plain LaTeX means no more fiddly `\backslash{curly brace}` all over the place. No more scattering code fragments and markup across hundreds of files. And on top of that, deep integration with my research notes and task tracking system.
...
This registers a new link type in Org Mode: `![cite:cohen93](cite:cohen93)`, which will jump to the appropriate bibliography entry when clicked, and get exported as `\cite{cohen93}` in LaTeX. Awesome!
